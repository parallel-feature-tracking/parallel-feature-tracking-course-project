...
def create_mpi_time_comparison_plot(data_dict, title, file_name):
    ...
    time_comparison_data.plot(kind='bar', logy=True)  # Set log scale for y-axis
    plt.title(title)
    plt.ylabel('Total Time Spent in MPI Functions (s) [Log Scale]')
...
# Creating the comparative visualization with log scale
create_mpi_time_comparison_plot(mpi_function_times_analysis, 
                                'Comparative Analysis of MPI Function Time Across Configurations (Log Scale)',
                                'mpi_func_time_comparison_log_scale.png')
